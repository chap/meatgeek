class Reading < ActiveRecord::Base
  ...
  def self.alert_air_temp
    ...
    if (ENV['ALERT_LOW'] && alarming_reading.temperature < ENV['ALERT_LOW'].to_i) || (ENV['ALERT_HIGH'] && alarming_reading.temperature > ENV['ALERT_HIGH'].to_i)
      ...
      if Reading.where(:probe_id => 2).where(:alerted => true).where("created_at >= ?", 10.minutes.ago).limit(1).first
        ...
        call = TWILIO_ACCOUNT.calls.create(
          ...
          :url => "http://twimlets.com/message?Message%5B0%5D=Dude%2C%20wake%20up.%20Air%20temperature%20is%20#{alarming_reading.temperature}!"
        )
